...
- **员工管理**：登录后打开 `/employees`，查看每名员工的 ID、姓名、token、二维码、检查数量、整改数量与状态；可点击「生成/刷新二维码」重新生成二维码，或「禁用」停用链接——停用后员工打开旧二维码 / 链接将无法查看与上传整改图（重新启用后可继续使用）。
- **员工端**：通过链接 `http://localhost:3000/fix?token=emp-token-001` 进入（或扫码），无需登录，查看待整改项（图片对按 #key 从小到大排序）并上传整改图。
- **汇总看板**：登录后打开 `/summary`，顶部可按员工筛选汇总，查看各员工整改进度与对比图；问题图与整改图成对展示，同一徽章（检查项+分值）共用。
